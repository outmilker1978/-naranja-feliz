...
-- subscription_requested_at column (referenced by code but never created)
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS subscription_requested_at TIMESTAMPTZ;

-- Course-level access: teacher grants access to specific course for a student
...
-- RPC: check if user has access to a specific course
