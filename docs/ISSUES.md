...
| #32 | Стратегическая оптимизация платформы для запуска к продаже + подготовки к Android (этапы B/C/D) | **Цель:** стабильно/быстро/доступно, подготовка к реальным платящим пользователям и мобильной версии. **B (производительность):** устранить N+1 в серверных роутах (уроки, уведомления, submissions), заменить поллинг чата 5с на supabase-js realtime, индексы БД (`chat_messages`,`chats`,`block_submissions` — скрипт в `docs/sql-chat-indexes.sql`), перевести клиентские запросы на сервер, убрать лишние `dangerouslySetInnerHTML`/пересоздание DOM-инпута. **C (надёжность/секьюрити):** роут-гварды, rate-limit, логирование ошибок на прод, аудит секретов/ключей. **D (Android prep):** PWA (+manifest+service worker, offline-кэш JS-чанков), проверка адаптивности/тап-тайтлов, API-first. Держать в бэклоге, идти по этапам A→B→C→D | бэклог |
