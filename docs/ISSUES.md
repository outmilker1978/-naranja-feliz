...
| #38 | Чат на 1 запрос + дедупликация layout (скорость дашборда) | См. RELEASE_NOTES v0.8.0 (подготовлен). Код: `/api/chat/init` (get_chat_data) + `src/lib/auth-cache.ts` (1×getUser + 1×profiles на запрос). `tsc`/`build` чисто | в работе (код готов, ждёт деплоя) |
...
- Проверка: `tsc --noEmit` чисто, `npm run build` успешен. **Деплой — по команде пользователя.**
