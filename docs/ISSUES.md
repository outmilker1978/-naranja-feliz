...
| #31 | Чат — автопрокрутка вниз к полю ввода каждые 1-2 сек (сброс прокрутки вверх) | `tools/chat/page.tsx:64` — `useEffect(..., [messages])` вызывает `bottomRef.scrollIntoView()` при КАЖДОМ обновлении сообщений. Сообщения обновляются поллингом каждые 5с (`setInterval(load, 5000)`), поэтому при любой прокрутке вверх пользователя через ~5с отматывает обратно вниз. **Фикс:** скроллить вниз только при (а) открытии чата/первой загрузке, (б) отправке собственного сообщения. При фоновом поллинге не сбрасывать позицию. Приоритет — высокий (явный UX-глюк) | в работе |
| #32 | Стратегическая оптимизация платформы для запуска к продаже + подготовки к Android (этапы B/C/D) | **Цель:** стабильно/быстро/доступно, подготовка к реальным платящим пользователям и мобильной версии. **B (производительность):** ✅ B1 — N+1 устранён через Postgres RPC (`supabase/rpc-aggregation.sql`): главная/курс/урок/список курсов переведены на 1 вызов (37→5 запросов), задеплоено v0.7.2; ✅ B3 timeout+retry; ✅ B4 логирование; ✅ B5 rate-limit; ✅ B6 секреты; 🔴 B7 кэш публичного контента (отложено). **C (надёжность):** 🔒 C1/C2 (аренда Yandex/Supabase). **D (Android prep):** ✅ D1 PWA (manifest+sw+offline); 🟠 D2 Android-обёртка (после стабилизации). **Осталось после деплоя B1:** подключить чат на `get_chat_data`, убрать дубли в layout (3×`auth.getUser`, 2×`profiles`), B7 кэш | бэклог |
| #33 | Проверка текущего пуша (`1ec6902`, `33ae819`, `9d29c7f`) — собрался ли на проде | Обложки/блоки/чат задеплоены пушем `300a397..33ae819`, но не оформлены в RELEASE_NOTES. Требует сверки активной ревизии Yandex Cloud с коммитами и подтверждения на проде + оформить RELEASE_NOTES (Этап A) | бэклог |
| #34 | Централизация запросов `block_submissions` на уроке — убрать N+1 медленных блоков (Порядок/Групповой порядок/Игра в пары) | Сейчас каждый интерактивный блок в `block-renderer.tsx` делал свой клиентский запрос к `block_submissions` (9 мест). У ученика на уроке 4 это растягивалось на минуты. **РЕАЛИЗОВАНО локально:** `types.ts` — тип `SavedByBlock`; `page.tsx` — один серверный запрос `.in("lesson_block_id", blockIds)` + прокинут `savedByBlock`; `block-renderer.tsx` — все 9 блоков инициализируются из `initialSubmission` вместо клиентского запроса (asinхронные запросы удалены); загрузка аудио в Storage (AudioAnswer) сохранена. `npm run build` проходит, `tsc --noEmit` чист. Бэкап: git-теги `backup-pre-*` + `backup-pre-opt\`. **ЗАКОММИЧЕНО** `771844e`. Протестировано пользователем — быстро. Ожидает деплоя | проверено |
| #31 | Чат — автопрокрутка вниз к полю ввода каждые 1-2 сек (сброс прокрутки вверх) | `tools/chat/page.tsx` — поллинг каждые 5с вызывал `scrollIntoView` и отматывал вниз. **РЕАЛИЗОВАНО:** скролл только при открытии чата и отправке своего сообщения (ref-флаг `pendingScrollRef` вместо state-in-effect), при фоновом поллинге прокрутка не сбрасывается. **ЗАКОММИЧЕНО** `771844e`. Протестировано пользователем — быстро. Ожидает деплоя. Приоритет высокий | проверено |
| #35 | Локальный dev тормозит «десятками секунд» — логин висит, страницы тормозят (НЕ код/индексы/компиляция) | ДИАГНОЗ замером (`/api/debug/perf`): зависал СЛУЧАЙНЫЙ запрос по 19–70с, остальные 60–200мс. Причина — «протухшие» HTTP keep-alive соединения к Supabase в долгоживущем процессе (ущемлённые мидлваре/auth/инлайн-клиенты). **ФИКС РЕАЛИЗОВАН полностью:** `src/lib/supabase/server.ts` — экспорт `supabaseFetch` (`Connection: close`); применён к `createClient/createAdminClient/createServiceClient`, а также к мидлваре (`middleware.ts`), всем auth-роутам (`login/signup/forgot-password/update-password`, `auth/callback`) и `tools-panel-wrapper`. Проверено: login = 351–451мс (было 70с), мидлваре = 258мс (было 19с), 8/8 data-запросов 1.2–1.5с без столов. `tsc` чист. **Локальный инструмент:** `GET /api/debug/perf?courseId=<id>`. **ЗАКОММИЧЕНО** `771844e`. Протестировано — быстро, без столов. Распространяется и на прод (надёжность). Ожидает деплоя | проверено |
| #36 | Оптимизация N+1 → Postgres RPC (этап B1) | **РЕАЛИЗОВАНО:** `supabase/rpc-aggregation.sql` — 5 функций (`get_home_data`, `get_course_page`, `get_lesson_page`, `get_chat_data`, `get_course_list`) + 14 индексов. Уроки/курс/главная/список курсов переведены на 1 RPC-вызов (37→5 последовательных запросов). RPC-функции применены в Supabase (SQL-эдитор). Проверено: урок с 13 блоками возвращается за ~0.2с одним вызовом, `tsc`/`build` чисты. Статус: страница скрытого (неопубликованного) урока корректно редиректит — не баг, а логика доступа. Ожидает деплоя в v0.7.2 | проверено |
