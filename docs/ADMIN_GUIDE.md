...
### check_course_access(uid uuid, cid uuid)
Возвращает true, если есть действующая запись в course_access (expires_at IS NULL OR expires_at > now()).

---

## Per-course доступ (по запросу)

### Для администратора/учителя

#### Создание курса с доступом «По запросу»
1. При создании/редактировании курса в поле **Доступ** выбери **«По запросу (учитель выдаёт доступ)»**
2. Сохрани курс — на карточке появится бейдж «По запросу»

#### Выдача доступа ученику
1. Меню -> **Учителя**
2. Найди ученика в списке, нажми **«Выдать доступ к курсам»**
3. В модалке: выбери курсы (чекбоксы), укажи срок в днях, нажми **«Выдать доступ»**
4. Ученик получит уведомление, курс появится в его списке с уроками

#### Отзыв доступа
В модалке напротив выданного курса нажми **«Отозвать»** — доступ закроется сразу.

#### Уведомления
При запросе ученика — уведомление всем учителям и админам. Клик открывает модалку с учеником и курсом.

#### Просмотр доступов
Зелёный бейдж — действует, красный — истёк.

#### Важно: владелец курса
Создатель курса (учитель) **всегда** имеет полный доступ к своему курсу в режиме ученика — проверка `check_course_access` для него пропускается. Это необходимо, чтобы учитель мог тестировать уроки без лишних шагов.
...
### Кредитная система доступа
- **«Дать доступ»** (подарок): `subscription_until += N дней`, долг не меняется.
- **«Дать доступ в кредит»**: `subscription_until += N дней`, долг (`credit_days`) += N.
- Оплата пакета сначала гасит долг (`credit_days -= min(долг, оплачено)`), остаток добавляется к сроку.
- Учитель может **списать долг** вручную в `/admin/teachers`.
- История операций в таблице `subscription_credit_history`, ученик видит её на `/settings`.

### Уведомления о подписке
- **За 5 дней / за 1 день / «закончилась»** — сайт-уведомление (→ `/settings`) + письмо на email (→ `/pricing`).
- **Мгновенно при входе** ученика и **ежедневно по cron** (06:00 UTC).
- Проверка: `/api/cron/subscription-expiry` (защищён `CRON_SECRET`). Если письмо не пришло — смотреть логи контейнера (`[subscription-reminder] SMTP fail ...`).

### Директор школы
- Админ назначает директора (одного) в `/admin/teachers`.
- Запросы продления подписки и доступа к курсу → директору; если директора нет → первому/всем учителям.
