import { Node, ReactNodeViewRenderer, NodeViewWrapper, NodeViewProps } from "@tiptap/react";
import { OrangeDivider } from "./orange-divider";

const OrangeDividerNodeView = ({ selected, node }: NodeViewProps) => {
  const align = node.attrs.textAlign || "center";
  return (
    <NodeViewWrapper style={{ textAlign: align, width: "100%" }}>
      <div className={`inline-flex items-center ${align === "center" ? "justify-center" : align === "right" ? "justify-end" : "justify-start"} ${selected ? "ring-2 ring-[#FF4D2D] rounded" : ""}`}>
        <OrangeDivider />
      </div>
    </NodeViewWrapper>
  );
...
export const OrangeDividerExtension = Node.create({
  ...
  addAttributes() {
    return {
      textAlign: { default: "center", parseHTML: el => el.style.textAlign || "center" },
    };
  },
  parseHTML() {
  ...
  renderHTML({ node }) {
    const align = node.attrs.textAlign || "center";
    const mx = align === "left" ? "0" : align === "right" ? "0 0 0 auto" : "auto";
    return [
      "div",
      { "data-type": "orange-divider", style: "display:flex;align-items:center;width:100%;height:2.5rem;margin:1.5rem 0" },
      [
        "svg",
        {
          width: "100%",
          ...
          style: `max-width:min(100%,480px);display:block;color:#F97316;margin:${mx}`,
        },
