...
## Алгоритм проверки инфраструктуры (ПЕРЕД ЛЮБЫМИ ДЕЙСТВИЯМИ)

**Если что-то не работает — НЕ ТРОГАТЬ вслепую. Сначала диагностика.**

1. **Что в гите VS что на проде:**
   - `git log --oneline -5` — последние коммиты
   - `yc serverless container revision list --container-id <id>` — активная ревизия + её тег
   - `yc serverless container revision get --id <active_revision> --format json` — env vars ревизии
   - Сравнить тег активной ревизии с последним коммитом. Если не совпадает — код не задеплоен.

2. **Проверка контейнера:**
   - Прямой URL: `curl https://<container>.containers.yandexcloud.net` → 200?
   - Если да — контейнер жив, проблема не в коде
   - Если нет — проверить env vars (п.1), откатиться на заведомо рабочую ревизию

3. **Проверка gateway:**
   - `curl https://naranja.outmilk.online` → 200? 502? 000?
   - Если 200 — всё ок
   - Если 502 — проблема в связке gateway→контейнер (проверить SA, container_id в spec)
   - Если 000 (таймаут) — проверить статус Yandex Cloud: https://status.yandex.cloud
   - Если оба своих gateway не отвечают — это Yandex Cloud, а не мы

4. **Перед откатом/сменой ревизии:**
   - Записать текущий ACTIVE revision id + image tag
   - Убедиться что новая ревизия будет иметь ВСЕ env vars (иначе 500)
   - Использовать `--environment` с полным списком переменных
   - Проверить что gateway работает ДО изменений

5. **Gateway recovery (если сломался):**
   - stop/resume — НЕ ДЕЛАТЬ (ломает gateway)
   - Вместо этого: удалить gateway → создать новый с тем же spec → прикрепить домен с существующим сертификатом
   - Сертификат уже есть: `fpqc35fc9ueg8u78gacv`
   - Gateway internal domain меняется при пересоздании
...
□ 3. Гигиена проекта:
   □ Упрощение контента: убрать дубли полей, мёртвый код, лишние category
   □ Мусор почищен: `.next/dev/`, пустые лог-файлы, неиспользуемые скрипты
   □ YCR — 2 последних тега, остальные удалены
   □ В коммит не попали ключи/секреты
   □ `git status` проверен — никаких случайных файлов
□ 4. Коммит + тег deploy-{epoch}
□ 5. Пуш + деплой (ТОЛЬКО после команды пользователя)

**Порядок не нарушать. Сначала docs + гигиена, потом деплой.**
