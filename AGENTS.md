...
## ЭКОНОМИЯ ТОКЕНОВ И ОРГАНИЗАЦИЯ СЕССИЙ (важно для стоимости и качества)
- **ИСТИНА В ФАЙЛАХ, не в чате.** Всё состояние проекта — в `docs/` (ISSUES, ARCHITECTURE, RELEASE_NOTES, ROADMAP) + `AGENTS.md`. Чат — только «рабочая доска», не хранилище. Новая сессия НИЧЕГО не теряет.
- **ОДНА ЗАДАЧА = ОДИН НОВЫЙ КОРОТКИЙ ЧАТ.** Длинный чат раздувает контекст → дорого (61% счёта — cache-hit) И ухудшает качество (модель деградирует, риск зацикливания — именно так сломался прошлый 3-месячный чат).
- Первый шаг новой сессии — процедура восстановления (читать AGENTS/ISSUES/ARCHITECTURE/RELEASE_NOTES) → «контекст восстановлен, готов работать». Это несколько тысяч токенов вместо сотен тысяч.
- При старте новой задачи пользователь формулирует: «продолжи по docs/ROADMAP §0, шаг X» — чтобы сразу знать направление.
- **Перед закрытием сессии — ОБЯЗАТЕЛЬНО закоммитить + обновить ISSUES/RELEASE_NOTES/ARCHITECTURE** (следующая сессия восстановится из них).
- Внутри сессии (правила экономии):
  - Читать файлы ТОЧЕЧНО: сначала `grep`/`glob`, потом `offset/limit`. НЕ читать целиком огромные файлы (блок-рендерер и т.п.) без крайней необходимости.
  - Обрезать большие выхлопы (лог build/dev, `git diff`) до ~10 строк; не тащить их в контекст.
  - Батчить вызовы в меньшее число запросов; не повторять диагностические циклы.
  - Писать кратко, без «воды».
- Приоритет: дешевизна и устойчивость длиннее, чем «не начинать заново».
...
## КОНТЕНТ ШКОЛЫ — НЕ ТРОГАТЬ (новое правило)
На сайте появился **реальный контент**, созданный учителем. Это святое:

- **Никогда не терять, не портить, не затирать** ни один учебный материал, контент портала, оформление
- Любая операция с БД — сначала проверить, что не затронет существующие записи
- Любой SQL-скрипт для миграции — пиши как `INSERT ... ON CONFLICT DO NOTHING` / `UPDATE WHERE`, никогда `DELETE` без точного WHERE по id
- Перед любыми манипуляциями с данными (даже в админке) — подумать «не сломаю ли я контент учителя?»

## Процедура работы (администратор + заказчик)
1. Пользователь приносит запрос (от лица заказчика/учителя)
2. Я вношу в `docs/ISSUES.md` с уникальным номером, статусом `бэклог`
3. Показываю таблицу, обсуждаем когда делать
4. После реализации — мои автоматические проверки + тест-кейс для пользователя (пошагово, со ссылками, ожидаемый результат)
5. Пользователь подтверждает результат → подготовка к деплою
6. Деплой: docs + RELEASE_NOTES + пошаговые инструкции (образ + БД)
...
## Полный регресс перед проверкой пользователем (ОБЯЗАТЕЛЬНО, всегда)
Перед любой сессией проверки функциональности у пользователя:
1. **Перезапустить dev-сервер начисто:** убить старые процессы (включая детей и занятый порт 3000), стартовать заново, дождаться HTTP 200 — чтобы не висели старые процессы/кэш.
2. **Проверить связку «урок↔курс» в данных** (REST к Supabase), чтобы все ссылки вели на реальные страницы; учебный контент не трогать.
3. **Выдать ПОЛНЫЙ регресс-план** одной нумерованной последовательностью шагов. Для каждого шага:
   - на каком аккаунте делать (учитель outmilk@yandex.ru / ученик outmilker@gmail.com);
   - кликабельная ссылка http://localhost:3000/...;
   - ЧТО сделать (простое действие);
   - ЧТО должно получиться (ожидаемый результат).
4. **Язык — простой, пользовательский,** без жаргона и внутренних номеров задач.
5. Один регресс покрывает ВСЕ правки текущего батча минимально достаточными шагами.
6. Пользователь проходит шаги по номерам и отвечает «номер шага — ок/не ок».
...
## Система TODO / Ишью-трекинг

### Файл
`docs/ISSUES.md` — единый файл всех ишью, статусов и истории.

### Команда `[TODO]` (от пользователя)

Когда пользователь пишет `TODO` или новую ишью:

1. **Прочитать** `docs/ISSUES.md` — восстановить контекст всех ишью
2. **Прочитать** `AGENTS.md` — восстановить правила работы
3. **Прочитать** `docs/ARCHITECTURE.md` — восстановить понимание проекта
4. **Прочитать** `docs/RELEASE_NOTES.md` — что уже сделано
5. **Показать таблицу** текущих ишью в формате:
   ```
   № | кратко суть | анализ исправления | статус
   ```
6. **Зарегистрировать** новую ишью с уникальным номером (следующий после последнего в ISSUES.md)
7. **Ждать команды** на начало работы

### Формат записи новой ишью
```
| #N | кратко | что делать / в чём баг | бэклог |
```

### Статусы
- `бэклог` — зарегистрировано, не начато
- `в работе` — фикс пишется
- `готово` — закоммичено, ждёт проверки
- `проверено` — пользователь подтвердил
- `релиз` — задеплоено на прод

### После обнуления (первая команда в сессии)
При первой команде в новой сессии:
1. Прочитать `AGENTS.md` (весь)
2. Прочитать `docs/ISSUES.md`
3. Прочитать `docs/ARCHITECTURE.md`
4. Прочитать `docs/RELEASE_NOTES.md`
5. Сказать пользователю «Контекст восстановлен, готов работать»

## ПРОТОКОЛ СЕССИИ (начало / конец) — долговременный проект
Источник истины = файлы (`docs/*`, `AGENTS.md`, git). Чат контекст НЕ хранит. Поэтому:

### Как НАЧИНАТЬ (новый короткий чат на задачу)
1. Юзер пишет первое сообщение-задание, напр.: «продолжи по `docs/ROADMAP.md` §0, шаг B1».
2. Агент ДОЛЖЕН: прочитать `AGENTS.md` (весь) → `docs/ISSUES.md` → `docs/ARCHITECTURE.md` → `docs/RELEASE_NOTES.md` (при необходимости `ROADMAP.md`).
3. Агент отвечает: «Контекст восстановлен, готов работать» + **краткий статус**: текущая версия, что в работе, открытые ишью, следующий шаг.

### Как ЗАКАНЧИВАТЬ (сохранить контекст для следующего чата)
1. Обновить `docs/ISSUES.md` — статусы сделанного, новые ишью, закрытые (лист должен сокращаться).
2. Обновить `docs/ARCHITECTURE.md` — если менялась инфраструктура/endpoints/ключи.
3. Обновить `docs/RELEASE_NOTES.md` — если есть изменения уровня релиза (или «готово к деплою»).
4. Обновить `docs/ROADMAP.md` — если менялся план/приоритет.
5. Коммит всего (ТОЛЬКО после команды юзера «делай/коммить»). Следить за чистотой дерева.
6. Дать юзеру **краткое резюме**: что сделано, что осталось, на чём продолжить (написать в новом чате).

### Доп.
- Лучше вести несколько КОРОТКИХ сессий по задачам, чем один мега-чат (дорого + риск цикла).
- Aдминистр. итог каждой сессии дублируется в суперпамять.
...
- **Ишью-трекинг** — после каждой сессии обновить `docs/ISSUES.md` (статусы, новые ишью, закрытые)
